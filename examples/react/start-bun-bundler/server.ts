/**
 * Production host for the Bun-bundler example.
 * Loads Vite-less Start output from dist/server + dist/client.
 */
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname.startsWith('/assets/') || /\.\w+$/.test(url.pathname)) {
      const file = Bun.file(
        `${CLIENT_DIR}${decodeURIComponent(url.pathname.replace(/^\//, ''))}`,
      )
